...
export function findEntries(mf) {
  if (Array.isArray(mf)) {
...
export default async function dom(html, { url: rawUrl, limit }) {
  ...
  const $ = element => {
    ...
    if (element.properties) {
      ...
      const content = element.properties.content ? element.properties.content[0].html : element.value;
